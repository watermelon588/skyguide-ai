...
const MODE_TONE = {
  local: { text: "text-ink-2", dot: "bg-ink-3" },
  lan: { text: "text-success", dot: "bg-success" },
  tunnel: { text: "text-accent", dot: "bg-accent" },
  production: { text: "text-accent", dot: "bg-accent" },
};

function Row({ label, value }) {
  return (
    <div className="flex items-baseline gap-2">
      <span className="w-16 shrink-0 text-[10px] uppercase tracking-wide text-ink-3">
        {label}
      </span>
...
export default function NetworkStatus() {
  ...
  return (
    <details className="fixed bottom-3 left-3 z-[60] w-[300px] max-w-[calc(100vw-1.5rem)] overflow-hidden border border-line bg-[#0B0D12]/90 shadow-2xl">
      <summary className="flex cursor-pointer list-none items-center gap-2 px-3 py-2 text-xs font-semibold text-ink select-none">
        <span className={`h-2 w-2 ${tone.dot}`} />
