 ...
 * orientation, or alignment yet — those are Session 4.
 */
export default function Align() {
  ...
  const { status, error } = usePairingSocket({
    roomId: room,
    token,
    role: "phone",
    enabled: validFormat,
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#090B12] px-6 text-white">
      ...
      >
        {validFormat ? (
          <>
            <PairingStatus status={status} error={error} />
          ...
          <div className="text-center">
            ...
            <p className="mt-4 text-sm leading-6 text-[#AAB4C5]">
              This link is missing a valid room or token. Please rescan the QR
              code from your SkyGuide dashboard.
            </p>
          </div>
        )}
      </motion.div>
